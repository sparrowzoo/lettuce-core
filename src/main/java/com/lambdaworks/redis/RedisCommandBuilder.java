...
class RedisCommandBuilder<K, V> extends BaseRedisCommandBuilder<K, V> {
    ...
    Command<K, V, List<String>> geohash(K key, V... members) {
        notNullKey(key);
        ...
        return createCommand(GEOHASH, new StringListOutput<>(codec), args);
    }
    ...
    @Deprecated
    Command<K, V, String> shutdown() {
        return createCommand(SHUTDOWN, new StatusOutput<>(codec));
    }
